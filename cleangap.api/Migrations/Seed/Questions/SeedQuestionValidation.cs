        ...
        private void SeedQuestionValidationSubmission(DAL.CleanGapDataContext context, question_sections qsHeader)
        {
            ...
            context.questions
                   .AddOrUpdate(
                        ...
                        new questions() { id = 125, name = "validation_submit_spec", description = "Are there specific order types that do not require inventory validation?", page = 31, parent_question_id = 124, parent_answer_value = "Yes", question_sections = qsHeader, id_section = qsHeader.id, id_subsection = subSection.id },
                        new questions() { id = 126, name = "validation_submit_cust", description = "Customer", page = 30, question_sections = qsHeader, parent_question_id = 120, parent_answer_value = "Yes", id_section = qsHeader.id, id_subsection = subSection.id }
            );
        }
    }
}